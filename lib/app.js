...
program.version(pkg.version).option('-p, --port <port>', 'REST service port', 3000).option('-b, --mongo <string>', 'MongoDB connection string', 'mongo://localhost/default').option('-m, --memcached <string>', 'Memcached servers list', '127.0.0.1:11211').option('-c, --cookie-name <string>', 'Unique identity cookie name', 'cntuid').option('--memcached-prefix <string>', 'Memcached keys prefix', 'storage_').parse(process.argv);
...
app.configure(function() {
  ...
  return app.set('cookieName', program.cookieName);
});
